...
verify it, and the hard rule that you flag but never edit the user's notes.

## The honest limits, stated to the user

State these plainly during Phase 0/2:
- You can check the **claims as written**. You **cannot** verify the user's
...
- LLM fact-checking is fallible, and most fallible on exactly the volatile
  ...
  Tell the user to verify anything important independently.
...
- **Do not edit the user's notes.**
- **Do not build a card from the disputed claim.**
- Surface it to the user with what you found and the source, and let them decide
  whether to research further, expand the note, or drop it.

This preserves the user as the author and avoids encoding a "correction" that may
itself be wrong.
...
and in many domains the older fact is exactly what the user means to retain.
...
- Add the current state *alongside* the user's fact (what's current, since when,
...
- Let the user decide what to card. They own the goal and know why X is in there.
- If you do card both, date and source each (see "Dating and sourcing" below).

**The failure mode to avoid:** silently rewriting the user's fact to the "modern"
...
second is, at most, an addition the user approves.
...
- Fill the `Date Added` field (YYYY-MM-DD) and a real `Source`.
