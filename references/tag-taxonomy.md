...
**Read the user's existing tag list first, and reuse exact strings.** Inconsistent
...
4. **Never a tag where a field/flag is better.** Difficulty → flag. Review date →
   the `Date Added` field. Importance → in-process signal (see card-principles §9).
...
6. **Tag at creation,** never as a later pass — retro-tagging thousands of cards
   is far harder.
7. **Prune with the source.** When the user revises a source note, find its cards
...
holds, and confirm with the user before creating it:
...
- it's a fundamentally different domain the user doesn't want mixed into daily
  review.
